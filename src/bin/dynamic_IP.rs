...
async fn main(spawner: Spawner) {
    ...
    let mut last_octet = [0u8; 8];
    rng.fill_bytes(&mut last_octet);
    
    let host = 1 + (last_octet[0] % 14);
    let new_IP1: Ipv4Address = Ipv4Address::new(10,42,0,host);

    info!("IP Nucleo1 is {}", new_IP1);
0

    let mut last_octet2 = [0u8; 8];
    rng.fill_bytes(&mut last_octet2);
    
    let host2 = 1 + (last_octet2[0] % 14);
    let new_IP2: Ipv4Address = Ipv4Address::new(10,42,0,host2);

    info!("IP Nucleo2 is {}", new_IP2);

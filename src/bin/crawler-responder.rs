...
async fn discovery_responder_task(stack: &'static Stack<'static>) -> ! {
    ...
    socket.bind(12345).unwrap(); // Same port as discovery request
    ...
    loop {
        ...
        match socket.recv_from(&mut payload_buf).await {
            Ok((n, sender)) => {
                if payload_buf[0] == DISCOVERY_REQUEST {
            ...
            Ok(_) => continue,
            Err(e) => {
                warn!("Discovery responder error: {:?}", e);
            }
        }
    }
}
...
async fn main(spawner: Spawner) {
    ...
    spawner.spawn(discovery_responder_task(stack)).expect("Failed to spawn responder");
